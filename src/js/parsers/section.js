...
export default class SectionParser {
  ...
  markupsFromElement(element) {
    ...
    if (this._isValidMarkupForElement(tagName, element)) {
      markups.push(this._createMarkup(tagName, getAttributes(element)));
    }
  ...
  _createMarkup(tagName, attributes) {
    let { builder } = this;
    attributes = filterObject(attributes, VALID_ATTRIBUTES);
    return builder.createMarkup(tagName, attributes);
  }
